...
class SQLFormatter {
    ...
    $keywordPattern = '/(\S)(\s*)(GROUP BY|ORDER BY|LIMIT|HAVING|UNION|UNION ALL)\b/i';
    ...
    private function formatWhereClause(string $sql): string {
        // Use a simple regex to find the entire WHERE clause
        if (!preg_match('/\bWHERE\b\s*(.*?)(?=\b(?:GROUP BY|ORDER BY|LIMIT|HAVING|UNION|$)|\Z)/is', $sql, $matches)) {
            return $sql; // No WHERE clause found
        }
        
        $originalWhereContent = $matches[1];
        $wherePosition = strpos($sql, 'WHERE');
        
        if ($wherePosition === false) {
            return $sql; // No WHERE found for some reason
        }
        
        // Simple token-based replacement approach that preserves everything
        $formattedWhere = $this->formatAndConditions($originalWhereContent);
        
        // Replace the original WHERE clause with our formatted version
        $beforeWhere = substr($sql, 0, $wherePosition);
        $afterWhere = substr($sql, $wherePosition + 5 + strlen($originalWhereContent));
        
        return $beforeWhere . 'WHERE' . $formattedWhere . $afterWhere;
    }

    private function formatAndConditions(string $whereClause): string {
        // This is a simpler approach that just looks for " AND " patterns directly
        // First normalize spaces around ANDs to make replacement reliable
        $whereClause = preg_replace('/\s+AND\s+/i', ' AND ', $whereClause);
        
        // Only proceed if we have AND conditions
        if (stripos($whereClause, ' AND ') === false) {
            return ' ' . $whereClause;
        }
        
        // Find all positions of " AND " (case insensitive)
        $positions = [];
        $pos = 0;
        while (($pos = stripos($whereClause, ' AND ', $pos)) !== false) {
            // Make sure this isn't part of another word
            $prevChar = ($pos > 0) ? $whereClause[$pos-1] : ' ';
            $nextChar = $whereClause[$pos+5];
            
            if (!ctype_alnum($prevChar) && !ctype_alnum($nextChar)) {
                $positions[] = $pos;
            }
            $pos += 5; // Move past this AND
        }
        
        // If no valid AND positions, return original
        if (empty($positions)) {
            return ' ' . $whereClause;
        }
        ...
        // Get the first condition (before first AND)
        $firstCondition = trim(substr($whereClause, 0, $positions[0]));
        $result .= ' ' . $firstCondition;
        
        // Add all the AND conditions with proper indentation
        foreach ($positions as $index => $pos) {
            $start = $pos + 5; // Skip past " AND "
            $end = isset($positions[$index+1]) ? $positions[$index+1] : strlen($whereClause);
            $condition = trim(substr($whereClause, $start, $end - $start));
            
            // Add formatted AND condition
            $result .= "\n" . $this->indentation . "AND " . $condition;
        }
